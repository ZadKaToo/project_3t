...
exports.loadLobby = async (req, res) => {
    try {
        ...
        const [inventory] = await db.query(`
            SELECT i.name, i.type, i.effect_value, i.description, inv.quantity 
            FROM Inventory inv
...
exports.loadMap = async (req, res) => {
    try {
        ...
        const [inventory] = await db.query(`
            SELECT i.name, i.type, i.effect_value, i.description, inv.quantity 
            FROM Inventory inv JOIN Items i ON inv.item_id = i.item_id WHERE inv.player_id = ?
        `, [playerId]);
...
exports.loadCombat = async (req, res) => {
    try {
        ...
        // 1. ดึงมอนสเตอร์จาก Session (ถ้ากำลังสู้อยู่) หรือสุ่มใหม่ (ถ้าเพิ่งเข้าฉาก)
        ...
        if (!currentMonster) {
            ...
            req.session.combatMonster = currentMonster; // เซฟลง Session
        }
        ...
        // 3. ดึง Log การต่อสู้ล่าสุด (ถ้ามี)
        const battleLogs = req.session.battleLogs || [];
        req.session.battleLogs = null; // เคลียร์ทิ้งหลังจากอ่านแล้ว

        res.render('combat', { 
            player: playerStats,
            monster: currentMonster,
            question: currentQuestion,
            battleLogs: battleLogs // ส่ง Log ไปแสดงผลที่หน้าจอ
        });
...
exports.executeCombat = async (req, res) => {
    try {
        ...
        // 🛠️ ปรับปรุงการเช็คคำตอบให้ครอบคลุม (แก้ปัญหา a เล็ก A ใหญ่ หรือมีช่องว่าง)
        let isCorrect = false;
        if (String(question_id) === '0') {
            isCorrect = true; // ข้อสมมติ
        ...
        // 🛡️ 4. ดึงสเตตัสโบนัสจากอาวุธและเกราะ
        let bonusDmg = 0;
        let bonusDef = 0;
        if (player.equipped_weapon) {
            const [wep] = await db.query('SELECT effect_value FROM Items WHERE item_id = ?', [player.equipped_weapon]);
            if (wep.length > 0) bonusDmg = wep[0].effect_value;
        }
        if (player.equipped_armor) {
            const [arm] = await db.query('SELECT effect_value FROM Items WHERE item_id = ?', [player.equipped_armor]);
            if (arm.length > 0) bonusDef = arm[0].effect_value;
        }

        // ⚔️ 5. คำนวณผลลัพธ์
        if (isCorrect) {
            logs.push(`<span class="text-green-400">[สำเร็จ] คุณตอบถูก! เวทมนตร์ทำงาน!</span>`);
            
            if (action === 'attack') {
                if (diceRoll === 20) {
                    const dmg = (10 + bonusDmg) * 2; 
                    monsterDmgTaken = dmg;
                    logs.push(`🎲 [เต๋า 20] 💥 คริติคอล!! คุณฟันเข้าจุดตาย (อาวุธบวก ${bonusDmg}) ทำดาเมจอย่างรุนแรงถึง ${dmg} หน่วย!`);
                } else if (diceRoll === 1) {
                    monsterDmgTaken = 0;
                    logs.push(`🎲 [เต๋า 1] 💀 ล้มเหลวคริติคอล! คุณสะดุดขาตัวเองล้ม โจมตีพลาดเป้าอย่างน่าอาย!`);
                } else {
                    const dmg = 10 + diceRoll + bonusDmg; 
                    monsterDmgTaken = dmg;
                    logs.push(`⚔️ คุณโจมตีเป้าหมาย (พลังโจมตี 10 + เต๋า ${diceRoll} + อาวุธ ${bonusDmg}) ทำดาเมจ ${dmg} หน่วย`);
                }

            } else if (action === 'defend') {
                const heal = Math.floor(diceRoll / 2) + 5;
                player.hp = Math.min(player.max_hp, player.hp + heal);
                logs.push(`🛡️ คุณร่ายเวทป้องกันสำเร็จ! ฟื้นฟูเลือด ${heal} หน่วย`);
                ...
                if (diceRoll >= 10) { 
                    ...
                    logs.push(`🏃 คุณพยายามหนี... แต่สะดุดล้ม! (ต้องการเต๋า 10+ แต่คุณได้ ${diceRoll})`);
                    const rawDmg = monster.damage;
                    playerDmgTaken = Math.max(0, rawDmg - bonusDef);
                    logs.push(`💥 ${monster.name} โจมตีคุณจากด้านหลัง ${rawDmg} หน่วย (เกราะกันได้ ${bonusDef}) เสีย HP ${playerDmgTaken}`);
                }
            }

        } else {
            // ❌ ตอบผิด 
            logs.push(`<span class="text-red-500">[ล้มเหลว] คุณตอบผิด! ความรู้ต้องห้ามกัดกินสมอง!</span>`);
            
            player.sanity = Math.max(0, player.sanity - 5);
            logs.push(`👁️ สติของคุณลดลง! (Sanity เหลือ ${player.sanity}/${player.max_sanity})`);

            let sanityPenalty = 0;
            if (player.sanity <= (player.max_sanity * 0.3)) {
                sanityPenalty = 10;
                logs.push(`<span class="text-purple-400">🌀 ภาพหลอนทำให้คุณกะระยะพลาด! คุณโดนโจมตีรุนแรงขึ้น!</span>`);
            }
            
            const rawDmg = monster.damage + Math.floor(diceRoll / 5) + sanityPenalty;
            playerDmgTaken = Math.max(0, rawDmg - bonusDef); 
            
            logs.push(`💥 ${monster.name} โจมตีใส่คุณ ${rawDmg} หน่วย (เกราะป้องกันได้ ${bonusDef}) คุณเสีย HP ${playerDmgTaken} หน่วย`);
        }

        // 🩸 6. หักเลือด
        if (monsterDmgTaken > 0) monster.hp -= monsterDmgTaken;
        if (playerDmgTaken > 0) player.hp -= playerDmgTaken;
        ...
        // 🔄 9. ถ้ายังไม่มีใครตาย อัปเดตข้อมูลลง DB & Session
...
exports.loadMap = async (req, res) => {
    try {
        ...
        // 1. ดึงข้อมูล Inventory (ตามที่คุณทำไว้แล้ว)
        const [inventory] = await db.query(`
            SELECT i.name, i.type, i.effect_value, i.description, inv.quantity 
            FROM Inventory inv JOIN Items i ON inv.item_id = i.item_id WHERE inv.player_id = ?
        `, [playerId]);
